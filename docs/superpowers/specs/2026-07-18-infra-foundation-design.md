...
All four nodes join one Tailscale tailnet. Private, encrypted, name-addressable, **zero open admin ports**. SSH, dashboards, backend-to-backend, and Alloy→Grafana traffic all ride the tailnet.

- **The Thousand Sunny (no sudo)** and **Going Merry (OpenVZ, maybe no `/dev/net/tun`)** run Tailscale in **userspace mode** (`tailscaled --tun=userspace-networking`) — no root required.
...
- **Komodo Core** on Thriller Bark; **Periphery agents** on Thriller Bark and Going Merry, reached over the tailnet.
...
- **Grafana** — dashboards.
- **Grafana Alloy** — one collector agent per node: scrapes container + host metrics and tails logs, shipping to VM/Loki over the tailnet. Runs as a userspace binary on nodes without Docker where feasible.
...
- **Add:** Tailscale, Caddy, SOPS/age, Komodo, Backrest (+restic/rclone), Grafana + VictoriaMetrics + Loki + Alloy.
...
- Replace the external `revproxy` network with Caddy-over-tailnet reverse proxying (drop the `revproxy` network from compose files).
...
- Exactly which arr services must run on Going Merry vs. Sunny, and how they reach the media library over the tailnet.
