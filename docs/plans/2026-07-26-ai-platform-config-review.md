...
| `RAG_TOP_K` / `RAG_TOP_K_RERANKER` | `40` / `8` | max recall |
| `RAG_RELEVANCE_THRESHOLD` / `RAG_HYBRID_BM25_WEIGHT` | `0.0` / `0.5` | defaults kept; scores aren't comparable across models |
| `RAG_RERANKING_ENGINE` | `external` → LiteLLM → Infinity on GM | `BAAI/bge-reranker-v2-m3` |
| `RAG_RERANKING_BATCH_SIZE` | `32` (default) | the knob if top-k 40 is slow on GM |
...
| new model entry | Infinity reranker via `infinity/` provider + `api_base` | |
...
| Infinity + `bge-reranker-v2-m3` | GM | CPU cross-encoder, x86 |
...
- Infinity rerank response shape through LiteLLM's `infinity/` provider — curl it.
